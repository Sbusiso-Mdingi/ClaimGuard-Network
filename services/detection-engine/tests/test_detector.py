...
class DetectorTests(TestCase):
    ...
    def test_directory_analysis_includes_network_sections(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(report["schemes"][0]["scheme_id"], "A")
            self.assertIn("provider_score_median", report["schemes"][0]["summary"])
            self.assertIn("member_score_median", report["schemes"][0]["summary"])
            self.assertIn("network", report)
            self.assertTrue(report["network"]["exact_banking_links"])
            self.assertTrue(report["network"]["behavioral_provider_links"])
            self.assertTrue(report["network"]["resolved_entities"])
            self.assertTrue(report["network"]["network_nodes"])
            self.assertIn("evaluation", report)
            self.assertTrue(report["evaluation"]["available"])
            self.assertIn("single_scheme", report["evaluation"])
            self.assertIn("cross_scheme", report["evaluation"])
            self.assertIn("detected", report["evaluation"]["single_scheme"])
            self.assertIn("total", report["evaluation"]["single_scheme"])
            self.assertIn("recall", report["evaluation"]["single_scheme"])
            self.assertIn("detected", report["evaluation"]["cross_scheme"])
            self.assertIn("total", report["evaluation"]["cross_scheme"])
            self.assertIn("recall", report["evaluation"]["cross_scheme"])
            self.assertIn("detection", report)
            self.assertIn("entities", report["detection"])
            self.assertIn("relationships", report["detection"])
            self.assertIn("triggered_rules", report["detection"])
