...
function canonicalMigrationVersion(schemaVersion) {
  const rendered = String(schemaVersion ?? "").trim();
  if (!/^[1-9]\d*$/.test(rendered)) {
    throw new Error("The operational schema version must be a canonical positive integer.");
  }
  return Number(rendered);
}

function organisationSafeDatabaseName(canonicalSlug) {
  const safe = String(canonicalSlug || "")
...
function organisationSecretPrefix(organisationId) {
  return `claimguard--tenant--${String(organisationId || "")
    .replace(/[^a-zA-Z0-9-]/g, "")
    .toLowerCase()}`;
}
...
function deterministicGuid(value) {
  const bytes = Buffer.from(
    crypto.createHash("sha256").update(value).digest().subarray(0, 16),
  );
...
async function grantReportWorkerSecretAccess(policy, secretNames) {
  ...
  if (!accessToken?.token) {
    throw new Error("Azure Resource Manager token acquisition failed.");
  }
  ...
  for (const secretName of secretNames) {
    ...
    if (!response.ok) {
      throw new Error(
        `Report-worker secret role assignment failed with HTTP ${response.status}.`,
      );
    }
  }
}
...
function parseAzurePolicy(organisation) {
  ...
  const schemaVersion = process.env.PRIVATE_TENANT_SCHEMA_VERSION?.trim()
    || CANONICAL_OPERATIONAL_SCHEMA_VERSION;
  const environmentKey = process.env.AZURE_APPROVED_ENVIRONMENT_KEY?.trim()
    || "production";
  const subscriptionId = requireEnv("AZURE_APPROVED_SUBSCRIPTION_ID");

  if (schemaVersion !== CANONICAL_OPERATIONAL_SCHEMA_VERSION) {
    throw new Error(
      `PRIVATE_TENANT_SCHEMA_VERSION must be ${CANONICAL_OPERATIONAL_SCHEMA_VERSION} for the canonical operational schema.`,
    );
  }
  ...
  return {
    ...
    reportContainer,
    schemaVersion,
    migrationVersion: canonicalMigrationVersion(schemaVersion),
    environmentKey,
    subscriptionId,
...
async function assertDatabaseUnclaimed(adminPool, policy) {
  ...
  const [metadataRows] = await adminPool.execute(
    `SELECT database_mode, logical_database_identifier
     FROM \`${policy.databaseName}\`.data_plane_metadata
  ...
  if (
    ...
    throw new Error(
      "Allocated database is already initialized for a different data-plane identity.",
    );
  }
}
...
async function createSecretStore() {
  ...
  const allowFallback = String(
    process.env.PROVISIONING_ALLOW_ENV_SECRET_FALLBACK || "false",
  ).toLowerCase() === "true";

  if (!keyVaultUri && !allowFallback) {
    throw new Error(
      "PROVISIONING_KEYVAULT_URI is required unless PROVISIONING_ALLOW_ENV_SECRET_FALLBACK=true.",
    );
  }
    ...
    return {
      ...
      async getSecret(name) {
        const value = process.env[
          `CLAIMGUARD_PROVISIONING_SECRET_${name.toUpperCase().replace(/[^A-Z0-9]/g, "_")}`
        ];
...
async function ensureTenantPrincipal(adminPool, { databaseName, username, password }) {
  ...
  try {
    ...
    await connection.query(
      `REVOKE ALL PRIVILEGES, GRANT OPTION FROM ${escapedUser}@'%'`,
    );
    await connection.query(
      `GRANT SELECT, INSERT, UPDATE, DELETE ON ${escapedDatabase}.* TO ${escapedUser}@'%'`,
    );
...
async function stepRunner(
  ...
  stepKey,
  runner,
  { resourceReference = null, leaseToken } = {},
) {
  ...
  await repositories.provisioning.startStep({
    operationId,
    stepKey,
  ...
  try {
    await runner();
    await repositories.provisioning.completeStep({
      operationId,
      stepKey,
      externalResourceReference: resourceReference,
    });
...
function generateTenantCredential(organisationId) {
  const slug = String(organisationId || "")
    .replace(/[^a-zA-Z0-9]/g, "")
    .slice(0, 20)
    .toLowerCase() || "tenant";
  return {
    username: `cg_runtime_${slug}`,
...
async function resolveTenantCredential(
  ...
  return { ...generateTenantCredential(organisationId), persisted: false };
}
...
async function runProvisioningOperation({
  ...
  if (!organisation) throw new Error("Organisation not found for operation.");
  ...
  await runStep("create_organisation_record", async () => {
    if (organisation.status !== "provisioning") {
      await service.transitionOrganisation(
        organisation.organisationId,
        "provisioning",
        { actor: { type: "system", id: "provisioning-worker" } },
      );
    }
  });
  ...
  await runStep("write_data_plane_metadata", async () => {
    const [rows] = await adminPool.execute(
      `SELECT database_mode, logical_database_identifier, schema_version,
         environment_key, migration_version
       FROM \`${policy.databaseName}\`.data_plane_metadata
       WHERE metadata_key = 'primary'`,
    );
    ...
    if (
      ...
      || Number(metadata.migration_version) !== policy.migrationVersion
    ) {
  ...
  await runStep("create_report_partition", async () => {
    await controlPool.execute(
      `INSERT INTO report_storage_partitions
        (partition_id, organisation_id, storage_type, logical_partition_key,
         resource_reference, provisioning_status, health_status, active_at)
       VALUES (?, ?, 'azure_blob_prefix', ?, ?, 'ready', 'unknown', UTC_TIMESTAMP(3))
       ON DUPLICATE KEY UPDATE
         resource_reference = VALUES(resource_reference),
    ...
    const [existingRows] = await controlPool.execute(
      `SELECT route_id FROM data_plane_routes
       WHERE organisation_id = ?
         AND route_type = 'private_database'
         AND schema_version = ?
         AND retired_at IS NULL
       ORDER BY route_generation DESC LIMIT 1`,
      [organisation.organisationId, policy.schemaVersion],
    );
  ...
  await runStep("record_schema_compatibility", async () => {
    ...
    await controlPool.execute(
      `UPDATE data_plane_routes
       SET provisioning_status = 'ready', health_status = 'healthy'
       WHERE route_id = ? AND active_route_slot IS NULL`,
      [routeId],
    );
    await controlPool.execute(
      `INSERT INTO organisation_schema_status
        (organisation_id, route_id, expected_schema_version,
         observed_schema_version, compatibility_status,
         last_checked_at, safe_error_summary)
       VALUES (?, ?, ?, ?, 'compatible', UTC_TIMESTAMP(3), NULL)
       ON DUPLICATE KEY UPDATE
         route_id = VALUES(route_id),
         expected_schema_version = VALUES(expected_schema_version),
         observed_schema_version = VALUES(observed_schema_version),
  ...
  await runStep("mark_report_worker_ready", async () => {
    await controlPool.execute(
      `UPDATE worker_routing_status
       SET status = 'ready', routing_generation = routing_generation + 1,
         safe_error_summary = NULL
  ...
  await runStep("create_initial_scheme_admin", async () => {
    const exists = await ensureInitialSchemeAdministrator(
      controlPool,
      organisation.organisationId,
    );
    if (!exists) {
      throw new Error(
        "No scheme administrator membership is registered for organisation.",
      );
    }
  });
  ...
  await runStep("run_activation_checks", async () => {
      ...
      async () => {
        const [rows] = await adminPool.execute(
          `SELECT COUNT(*) AS count
           FROM \`${policy.databaseName}\`.data_plane_metadata
           WHERE metadata_key = 'primary'
             AND database_mode = 'private_database'
             AND logical_database_identifier = ?
             AND schema_version = ?
      ...
      async () => {
        const [rows] = await controlPool.execute(
          `SELECT COUNT(*) AS count FROM data_plane_routes
           WHERE organisation_id = ?
             AND route_type = 'private_database'
             AND active_route_slot IS NOT NULL`,
          [organisation.organisationId],
        );
        return Number(rows?.[0]?.count || 0) === 0;
      },
    ...
    for (const check of checks) {
      if (!await check()) throw new Error("Activation checks failed.");
    }
  });
...
async function runUpgradeOperation({
  ...
  if (operation.operationType !== "upgrade_private_database") {
    throw new Error("Unsupported upgrade operation type.");
  }
  ...
  if (!tenantCredential.persisted) {
    throw new Error("Existing private database credentials were not found in Key Vault.");
  }
  ...
  await runStep("validate_upgrade_request", async () => {
    if (
      ...
      || !["active", "suspended", "ready_for_activation"].includes(
        organisation.status,
      )
    ) {
    ...
    const [rows] = await controlPool.execute(
      `SELECT * FROM data_plane_routes
       WHERE organisation_id = ?
         AND route_type = 'private_database'
         AND retired_at IS NULL
       ORDER BY (active_route_slot = organisation_id) DESC,
         route_generation DESC LIMIT 2`,
      [organisation.organisationId],
    );
    sourceRoute = rows?.[0];
    if (
      !sourceRoute
      || sourceRoute.database_name !== policy.databaseName
      || sourceRoute.logical_database_identifier !== policy.logicalDatabaseIdentifier
    ) {
      throw new Error(
        "The existing private route does not match the approved database identity.",
      );
    }

    const expectedReferences = secretNames.map(
      (name) => `https://${policy.keyVaultName}.vault.azure.net/secrets/${name}`,
    );
    const actualReferences = String(sourceRoute.secret_reference || "")
      .split(",")
      .map((value) => value.trim());
    if (expectedReferences.some(
      (reference, index) => actualReferences[index] !== reference,
    )) {
      throw new Error(
        "The existing private route secret references do not match the approved tenant secrets.",
      );
    }
  });
  ...
  if (!sourceRoute) {
    const [rows] = await controlPool.execute(
      `SELECT * FROM data_plane_routes
       ...
       ORDER BY (active_route_slot = organisation_id) DESC,
         route_generation DESC LIMIT 1`,
      [organisation.organisationId],
    );
    sourceRoute = rows?.[0];
  }
  if (!sourceRoute) {
    throw new Error("Private route disappeared after upgrade validation.");
  }
  ...
  await runStep("register_canonical_schema_route", async () => {
    ...
    if (existingRows?.[0]?.route_id) {
      upgradedRoute = await repositories.routes.getSafeById(
  ...
  await runStep("record_schema_compatibility", async () => {
    ...
    if (!upgradedRoute?.routeId) {
      throw new Error(`Schema-${policy.schemaVersion} route was not registered.`);
    }
