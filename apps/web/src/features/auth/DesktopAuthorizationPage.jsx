...
export function DesktopAuthorizationPage() {
  ...
  if (!workforce.isSignedIn) {
    return <main className="grid min-h-screen place-items-center bg-background p-5"><SignIn routing="virtual" fallbackRedirectUrl={window.location.href} signUpUrl="/sign-up" /></main>;
  }
  if (!workforce.organisationId) {
    return <main className="grid min-h-screen place-items-center bg-background p-5"><Card className="w-full max-w-lg"><CardHeader><CardTitle>No work organisation selected</CardTitle><CardDescription>Sequrin does not allow users to create organisations. Sign out, then use the governed Clerk invitation for the organisation licensed on this workstation.</CardDescription></CardHeader><CardContent><Button variant="outline" onClick={() => workforce.signOut?.({ redirectUrl: window.location.href })}>Sign out</Button></CardContent></Card></main>;
  }
    ...
    <main className="grid min-h-screen place-items-center bg-background p-5 text-foreground">
      <Card className="w-full max-w-xl">
        ...
        <CardContent className="space-y-5">
          ...
          {state.status !== "approved" ? <Button className="w-full" variant="outline" onClick={() => workforce.signOut?.({ redirectUrl: window.location.href })}>Use a different work account</Button> : null}
        </CardContent>
      </Card>
    </main>
  );
