...
function assertPool(c) {
  const services = getOperationalServices();
  const pool = services?.operationalRepositories?.dataPlaneContext?.pool
    || services?._pool
    || null;
  if (!pool) {
    const error = new Error("Operational data-plane pool is not available.");
    error.status = 503;
    error.code = "DATA_PLANE_POOL_UNAVAILABLE";
    throw error;
  }
  return pool;
}

function getDataPlanePool(c) {
  // Access the pool from the data-plane context set by the middleware.
  const ctx = c.get("dataPlaneContext");
  if (ctx?._pool) return ctx._pool;
  const services = getOperationalServices();
  return services?._pool || null;
}
...
export function registerAssessmentRoutes(app, {
  ...
  app.post(
    ...
    async (c) => {
      const services = getOperationalServices();
      if (!services?.operationalRepositories) {
        return c.json({ code: "DATA_PLANE_UNAVAILABLE", message: "Operational data plane is not available." }, 503);
      }
      const tenantContext = c.get("tenantContext") || null;
      if (!tenantContext?.tenant_id) {
      ...
      const pool = services.operationalRepositories.dataPlaneContext?._pool
        || services._pool
        || null;
      if (!pool) {
        return c.json({ code: "DATA_PLANE_UNAVAILABLE", message: "Operational pool is not available." }, 503);
      }
