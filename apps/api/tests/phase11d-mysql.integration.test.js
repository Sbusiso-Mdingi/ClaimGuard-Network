  ...
  await pool.execute(
    `
      ...
      ON DUPLICATE KEY UPDATE
        ...
        rendering_practitioner_category =
          VALUES(rendering_practitioner_category),
        rendering_known_to_billing_provider =
          VALUES(
            rendering_known_to_billing_provider
          ),
...
async function seedOperationalFixtures(
  ...
  /*
   * Clear only prospective records created by a previous
   * interrupted execution of this integration test.
   */
  await pool.execute(
    `
  ...
  /*
   * Migration 0014 creates strategies for tenants that
   * already exist during migration. These fixtures are
   * inserted afterward, so create their explicit active
   * prospective strategies here.
   */
  await pool.execute(
    `
