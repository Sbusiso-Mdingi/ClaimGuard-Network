...
test("shadow credential accepts nullable hash, scopes username, and safe DTO hides hash", async () => {
  ...
  const db = executor(async (sql, params) => {
    if (sql.startsWith("INSERT INTO credential_identities")) {
      stored = {
        ...
        user_id: params[1],
        organisation_id: params[2],
        authentication_provider: params[3],
        normalized_username: params[4],
    ...
    if (sql.startsWith("SELECT * FROM credential_identities WHERE credential_id")) {
      return [[stored], []];
    }
  ...
  await assert.rejects(
    () => repositories.identity.createCredential({
      userId: "u",
      organisationId: "o",
      username: "x",
      password: "unsafe",
...
test("session foundation stores only a hash and safe projection excludes it", async () => {
  ...
  const db = executor(async (sql, params) => {
    ...
    if (sql.startsWith("SELECT * FROM login_sessions")) {
      return [[stored], []];
    }
    throw new Error(`Unexpected SQL: ${sql}`);
  });
...
test("route registration increments generation and safe routes omit secret references", async () => {
  let stored = null;
  const db = executor(async (sql, params) => {
    if (sql.startsWith("SELECT COALESCE(MAX(route_generation)")) {
      return [[{ next_generation: 4 }], []];
    }
    if (sql.startsWith("UPDATE data_plane_routes")) {
      return [{ affectedRows: 1 }, []];
    }
    if (sql.startsWith("INSERT INTO data_plane_routes")) {
      stored = {
        route_id: params[0],
        organisation_id: params[1],
        route_type: params[2],
        logical_database_identifier: params[3],
        secret_reference: params[6],
        region: params[7],
        route_generation: params[8],
        schema_version: params[9],
        provisioning_status: params[10],
        health_status: params[11],
        active_at: params[12],
      };
      return [{ affectedRows: 1 }, []];
    }
    if (sql.startsWith("SELECT * FROM data_plane_routes WHERE route_id")) {
      return [[stored], []];
    }
    throw new Error(`Unexpected SQL: ${sql}`);
  });
  ...
  const route = await repository.register({
    ...
    logicalDatabaseIdentifier: "legacy-operational",
    databaseName: "legacy",
    secretReference: "kv://legacy-db",
    activate: true,
  });
  ...
  await assert.rejects(
    () => repository.register({
      organisationId: "org-1",
      routeType: "legacy_shared",
      logicalDatabaseIdentifier: "x",
      secretReference: "mysql://u:p@h/d",
    }),
...
test("authentication events accept safe hashes and preserve correlation IDs", async () => {
  ...
  const event = await repository.recordAuthenticationEvent({
    ...
    organisationCandidateHash: "a".repeat(64),
  ...
  await assert.rejects(
    () => repository.recordAuthenticationEvent({ eventType: "unknown" }),
    /Unsupported/,
  );
  await assert.rejects(
    () => repository.recordAuthenticationEvent({
      eventType: "login_failure",
      password: "unsafe",
    }),
    /not permitted/,
...
test("users are platform-level and one user can hold memberships in multiple organisations", async () => {
  ...
  const db = executor(async (sql, params) => {
    ...
    if (sql.startsWith("SELECT * FROM organisation_memberships")) {
  ...
  const first = await identity.createMembership({
    membershipId: "m-1",
    userId: user.userId,
    organisationId: "org-1",
    status: "active",
  });
  const second = await identity.createMembership({
    membershipId: "m-2",
    userId: user.userId,
...
test("future active membership repair is exact and uses database UTC", async () => {
  const membership = {
    ...
    status: "active",
    valid_from: new Date("2026-07-30T06:49:36.719Z"),
  };
  const calls = [];
  const db = executor(async (sql, params) => {
    calls.push({ sql, params });
    if (sql.startsWith("UPDATE organisation_memberships SET valid_from = UTC_TIMESTAMP(3)")) {
      membership.valid_from = new Date("2026-07-30T05:15:00.000Z");
      return [{ affectedRows: 1 }, []];
    }
    if (sql.startsWith("SELECT * FROM organisation_memberships")) {
      return [[membership], []];
  ...
  const repaired = await createControlPlaneRepositories(db)
    .identity
  ...
  assert.match(calls[0].sql, /valid_from > UTC_TIMESTAMP\(3\)/);
  assert.deepEqual(
    calls[0].params,
    ["membership-2", "user-2", "org-platform"],
  );
});
...
test("identity repository resolves an existing invitation user and organisation membership", async () => {
  ...
  const identity = createControlPlaneRepositories(db).identity;
  const user = await identity.getSafeUserByCanonicalContact(
    " ADMIN@UBUNTU.EXAMPLE ",
    { lockForUpdate: true },
  );
  const membership = await identity.getMembershipForUserOrganisation(
    {
      userId: user.userId,
  ...
  assert.equal(user.userId, "user-existing");
  assert.equal(user.canonicalContact, "admin@ubuntu.example");
  assert.equal(membership.membershipId, "membership-existing");
  assert.equal(membership.organisationId, "org-ubuntu");
  assert.deepEqual(calls[0].params, ["admin@ubuntu.example"]);
  assert.deepEqual(calls[1].params, ["user-existing", "org-ubuntu"]);
...
test("repository conflicts preserve typed duplicate slug and username errors", async () => {
  ...
  const db = executor(async (sql) => {
    if (
      sql.startsWith("INSERT INTO organisations")
      || sql.startsWith("INSERT INTO credential_identities")
    ) {
      throw duplicate;
    }
    throw new Error(`Unexpected SQL: ${sql}`);
...
test("provisioning step completion is repeatable and failures store classified safe metadata", async () => {
  ...
  const first = await provisioning.completeStep({
    operationId: "op",
    stepKey: "schema",
  });
  const repeated = await provisioning.completeStep({
    operationId: "op",
    stepKey: "schema",
  });
  const failed = await provisioning.failStep({
    ...
    stepKey: "route",
    error: Object.assign(
      new Error("mysql://user:secret@host/db"),
      { code: "ER_CONNECT" },
    ),
  });
  ...
  assert.deepEqual(
    failureCall.params.slice(-2),
    ["Error", "Error:ER_CONNECT"],
  );
  assert.equal(
    failureCall.params.some((value) => String(value).includes("secret@host")),
    false,
  );
});
