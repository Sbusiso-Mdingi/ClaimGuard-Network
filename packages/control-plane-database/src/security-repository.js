...
const AUTH_EVENT_TYPES = new Set([
  ...
  "session_expired", "authentication_version_mismatch", "authorization_version_mismatch", "header_spoof_attempt", "csrf_rejection",
  "reauthentication_success", "reauthentication_failure",
  "external_authentication_success", "external_authentication_failure",
]);
...
function assertVersion(value, fieldName) {
  ...
  if (!Number.isInteger(version) || version < 1) throw new TypeError(`${fieldName} must be a positive integer.`);
  return version;
}
      ...
      await executorOr(defaultExecutor, executor).execute(
        `INSERT INTO login_sessions
          (session_id, hashed_bearer_secret, csrf_token_hash, signing_key_id, user_id, organisation_id, membership_id,
           issued_at, last_activity_at, idle_expires_at, absolute_expires_at, authentication_version,
           authorization_version, client_metadata)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [sessionId, assertHash(input.hashedBearerSecret, "hashedBearerSecret"),
          ...
          assertVersion(input.authenticationVersion, "authenticationVersion"),
          assertVersion(input.authorizationVersion, "authorizationVersion"),
