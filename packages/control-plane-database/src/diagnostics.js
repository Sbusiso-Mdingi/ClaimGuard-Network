...
export async function getShadowDiagnostics(pool) {
  ...
  const [organisationCount, mappingCount, missingRoutes, missingAdministrators, invalidRoleAssignments, slugConflicts, schemaMismatches] = await Promise.all([
  ...
  return {
    ...
    problems: { missingRoutes, missingAdministrators, invalidRoleAssignments, slugConflicts, schemaMismatches },
  };
}
