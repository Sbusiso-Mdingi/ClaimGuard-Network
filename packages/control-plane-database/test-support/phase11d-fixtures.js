...
const SCHEME_TEST_ROLES = Object.freeze([
  ...
  ["investigator", "investigator.phase11d", "Investigator"],
]);

const FIXTURE_ACTOR = Object.freeze({
  type: "system",
  id: "phase11d-fixture",
  source: "phase11d-fixture",
});
...
async function activateOrganisation(service, organisation) {
  ...
  const nextByStatus = {
    draft: "provisioning",
    failed: "provisioning",
    provisioning: "ready_for_activation",
    ready_for_activation: "active",
  };

  while (nextByStatus[current.status]) {
    current = await service.transitionOrganisation(
      current.organisationId,
      nextByStatus[current.status],
    );
  }

  if (current.status !== "active") {
    throw new Error(
      `Integration organisation ${current.canonicalSlug} cannot be activated from ${current.status}.`,
    );
  }
...
async function ensureOrganisation({
  ...
  if (
    organisation.deploymentClass !== "demo"
    || organisation.organisationType !== organisationType
  ) {
    throw new Error(
      `Existing organisation ${slug} is not an eligible ${organisationType} integration organisation.`,
    );
  }
...
async function ensureRoute({
  ...
  const existing = await repositories.routes.getSafeActiveForOrganisation(
  ...
  if (existing) {
    const schemaMatches =
      routeType === "platform_none"
        ...
        : String(existing.schemaVersion ?? "") === expectedSchemaVersion;
    ...
    if (
      ...
      throw new Error(
        `Active route for ${organisation.canonicalSlug} is not an eligible schema-${expectedSchemaVersion ?? "none"} ${routeType} route.`,
      );
    }
...
async function ensureVerifiedMapping({
  executor,
  organisation,
  tenant,
  route,
  ...
  const [rows] = await executor.execute(
    "SELECT * FROM legacy_tenant_mappings WHERE organisation_id = ? LIMIT 1",
    [organisation.organisationId],
  );
  ...
  if (
    existing
    && (
      existing.legacy_tenant_id !== tenant.tenantId
      || existing.legacy_tenant_slug !== tenant.tenantSlug
    )
  ) {
    throw new Error(
      `Legacy mapping for ${organisation.canonicalSlug} conflicts with the operational tenant inventory.`,
    );
  }
...
async function ensureAccount({
  ...
  if (credential) {
    ...
    await repositories.authentication.revokeSessionsBy(
      "user",
      userId,
      "password_changed",
    );
    await executor.execute(
      ...
      [
        passwordHash,
        JSON.stringify(passwordParametersRecord()),
        ARGON2ID_VERSION,
        credential.credentialId,
      ],
    );
    membership = await repositories.authentication.getMembership({
      userId,
      organisationId: organisation.organisationId,
    });
    ...
    if (!membership) {
      throw new Error(
        `Credential ${username} has no membership in its organisation.`,
      );
    }
    ...
    const user = await repositories.identity.createUser({
      displayName: `${roleLabel} Integration User`,
      canonicalContact: `${username}.${organisation.canonicalSlug}@test.invalid`,
      status: "active",
    });
    userId = user.userId;
  ...
  return {
    organisation: organisation.canonicalSlug,
    role: roleKey,
    username,
    password,
  };
...
export async function provisionPhase11dFixtures({
  tenants,
  repositories,
  service,
  executor,
  operationalDatabaseName = null,
}) {
  if (!Array.isArray(tenants) || tenants.length === 0) {
    throw new Error("At least one operational integration tenant is required.");
  }
  ...
  for (const tenant of tenants) {
    ...
    for (const [roleKey, username, roleLabel] of SCHEME_TEST_ROLES) {
  ...
  oneTimeCredentials.push(
    await ensureAccount({
      ...
      organisation: platform,
      roleKey: "platform_administrator",
      username: "platform.admin.phase11d",
